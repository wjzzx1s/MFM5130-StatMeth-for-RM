...
\begin{align*}
    ...
    &\ES_{\alpha}(L) = \frac{\E(L\mathbbm{1}_{\{L>F_L^{\leftarrow}(\alpha)\}})+F_L^{\leftarrow}(1-\alpha-\bar{F_L}(F_L^{\leftarrow}(\alpha)))}{1-\alpha}.
\end{align*}
...
\begin{align*}
    ...
    \begin{cases}
        ...
        \inf\{y \in \real:F_L(y + F_L^{\leftarrow}(\alpha)) \le u\}, & F_L(F_L^{\leftarrow}(\alpha)) \le u \le 1,
    \end{cases}\\
    &=
    \begin{cases}
    ...
    \end{cases}
\end{align*}
It can be noticed that when $\alpha \le u \le F_L(F_L^{\leftarrow}(\alpha))$, $F_L^{\leftarrow}(u) = F_L^{\leftarrow}(\alpha)$. The above fomula can be also written as
\begin{align*}
    F_Y^{\leftarrow}(u) = 
    \begin{cases}
        0, &0 \le u <  \alpha,\\
...
\begin{align*}
    ...
    &=\frac{\E(L\mathbbm{1}_{\{L>F_L^{\leftarrow}(\alpha)\}})+F_L^{\leftarrow}(1-\alpha-\bar{F_L}(F_L^{\leftarrow}(\alpha)))}{1-\alpha}, 
\end{align*}
where
\[
...
\[
    \VaR_{\alpha}(L) = \VaR_{\alpha}(\mu + \sigma X) = \mu + \sigma \VaR_{\alpha}(X) = \mu + \sigma \Phi^{-1}(X).  
\]
\[
...
By L'Hôpital's rule,
\[
    \lim\limits_{\alpha \rightarrow 1^{-}} \frac{\ES_{\alpha}(X)}{\VaR_{\alpha}(X)} = \lim\limits_{u \rightarrow + \infty} \frac{\varphi(u)}{u[1-\Phi(u)]} = \lim\limits_{u \rightarrow + \infty} \frac{-u\varphi(u)}{1-\Phi(u)-u\varphi(u)} = \lim\limits_{u \rightarrow + \infty} \frac{(u^2-1)\varphi(u)}{(u^2-2)\varphi(u)} = 1,
\]
hence
\[
...
Because $F_L(x)$ is continuous and strictly increasing, $F_L(t)^{\leftarrow} = F_L^{-1}(t)$ for any $t \in (0,1)$. Let $u = \VaR_{\alpha}(L) = F_L^{-1}(\alpha)$. Then when $\alpha \rightarrow 1^{-}$, $\VaR_{\alpha}(L) \rightarrow +\infty$ and $\ES_{\alpha}(L) \rightarrow + \infty$. We have
\[
    \ES_{\alpha}(L) = \frac{1}{1-\alpha}\int_{\alpha}^1F_L^{-1}(t)\dd t \stackrel{t\triangleq F_L^{-1}(t)}{=\joinrel=\joinrel=\joinrel=\joinrel=\joinrel=} \frac{1}{1-F_L(u)}\int_{u}^{+\infty} tf_L(t)\dd t,
\]
thus for $\nu > 1$
\begin{align*}
...
\textbf{Proof: } The probability distribution function of $L_1$ is
\[
...
\textbf{Proof: }the expectation of moment generating function is
\begin{align*}
    \E(e^{tL_1}) = \int_1^{+\infty} e^{tx} f_L(x) \dd x = \int_1^{+\infty} e^{tx} \frac12x^{-\frac{3}{2}} = \frac12 \int_1^{+\infty} x^{-\frac32}e^{tx}\dd x,
\end{align*}
where the integrand diverge to $+\infty$ for all $ t > 0$ when $x \rightarrow +\infty$, and hence $\E(e^{tL_1}) = +\infty$ and $L_1$ has a heavy tail. \qed \\
...
\[
    q = \frac{1}{(1-\alpha)^2},
\]
by definition of value at risk, $\P(L_1+L_2 > p) = 2\sqrt{x-1}/x = 1- \alpha, \quad p \ge 2$, which can derive that
\[
...
\[
    p' = \frac{2(1-\sqrt{2\alpha-\alpha^2})}{(1-\alpha)^2}
\]
is eliminated because $p \ge 2$. But $p' \ge 2$ only derives that
\[
    \alpha = 0 \text{ or }\alpha = 2,    
\]
which contradicts $\alpha \in (0,1)$.
...
\HWproblem
(Skewness)In the market, there are 100 independent bonds $L_i,i\in\{1,2,\cdots,100\}$ with the same nominal value of 100, 2\% yearly coupon, 1\% default probability and no recovery, i.e. $L_i, i\in\{1,2,\cdots,100\}$ independently following the below distributions
\[
    L_i = 
    \begin{cases}
    ...
    \end{cases}
\]
An investor would like to buy 100 bonds and hopes that the risk can be managed. There are two strategies(portfolios) for the investor to consider:\\
...
\textbf{Solution: }Let $F_1(x), F_2(x)$ be the cumulated distribution function of $P_1$ and $P_2$. $\mathcal{P}_2$ has only two possible values -200 and 10000, and
\[
...
Let $\mathcal{D}$ be the number of defaulted bonds, then $\mathcal{D} \sim B(100, 0.01)$, i.e. the binomial distribution with parameters 100 and 0.01. And $\mathcal{P}_1 = -2(100-\mathcal{D})+100\mathcal{D} = 102\mathcal D - 200$. Calculate the probablity mass function(PMF) and cumulated distribution function(CDF) for the first few $\mathcal{D}$'s possible values.
\begin{table}[h]
    \centering
    \renewcommand{\arraystretch}{1.5} % 将行高放大至 1.5 倍
    \setlength{\tabcolsep}{20pt}     % 将列与列之间的间距增加到 20pt
